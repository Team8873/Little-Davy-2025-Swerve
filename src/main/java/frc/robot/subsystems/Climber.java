...
public class Climber extends SubsystemBase { // puts climber as a subsystem; inside is code for the climber
    // introduce stuff
    private final SparkMax motorForClimber = new SparkMax(ClimberConstants.motorForClimberCANID, MotorType.kBrushless);// a
                                                                                                                       // motor
                                                                                                                       // motorForClimber
    private final DutyCycleEncoder encoderForClimber = new DutyCycleEncoder(ClimberConstants.encoderForClimberDIOPort); // through-bore
                                                                                                                        // encoder
    ...
    private GenericEntry positionEntry = tab.add("Climber Position", 0)
            .withWidget(BuiltInWidgets.kNumberBar)
            .withPosition(9, 1)
            .getEntry();
    private GenericEntry targetEntry = tab.add("Climber target", 0)
            .withWidget(BuiltInWidgets.kNumberBar)
            .withPosition(9, 2)
            .getEntry();
    private GenericEntry statusEntry = tab.add("Servo Status", 0)
            .withWidget(BuiltInWidgets.kNumberBar)
            .withPosition(9, 3)
            .getEntry();
    private GenericEntry speedEntry = tab.add("Climber Speed", 0)
            .withWidget(BuiltInWidgets.kNumberBar)
            .withPosition(9, 4)
            .getEntry();

    // SERVOSTUFF
    // need to trip the servo to move the motor in positive direction. 1.0 is
