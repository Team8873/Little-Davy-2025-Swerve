...
public class Arm extends SubsystemBase {
  ...
  private final SparkMax armMotor = new SparkMax(ArmConstants.armCanId, MotorType.kBrushless); // create the motor
                                                                                               // object
  ...
  private final PIDController wristPid = new PIDController(ArmConstants.wristkP, ArmConstants.wristkI,
      ArmConstants.wristkD);
  ...
  public Arm() {
    ...
    tab.addDouble("wrist position", ()-> wristPosition).withWidget(BuiltInWidgets.kNumberBar).withPosition(0, 4);
    tab.addDouble("arm position", ()-> armPosition).withWidget(BuiltInWidgets.kNumberBar).withPosition(1, 3);
  ...
  // creates a trigger for armMech Status that can be used to automatic run
  // command when true
